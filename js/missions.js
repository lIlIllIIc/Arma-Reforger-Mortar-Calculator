...
function calculateMission(missionType, bypassNFA = false) {
    ...
    // 1 section → familiar single Base/Corrected boxes. 2+ → per-section cards.
    ...
    // SVG mini-map showing mortars, target, sheaf points and nearby NFAs.
...
function renderMiniMap(missionType, target, gunSolutions) {
    ...
    // Collect mortar positions (with section labels for tooltips).
    const mortars = [];
...
function renderMissionSolutions(missionType, sectionSolutions) {
    ...
    sectionContainer.innerHTML = renderSectionSolutionsHTML(sectionSolutions, missionType);
